...
public class CreateDesignCommandValidatorTests
{
    private readonly CreateDesignCommandValidator _validator = new();
    ...
    // --- Coincident ball joints ---

    [Fact]
    public void CoincidentBallJoints_ShouldFail()
    {
        var samePoint = new Point3DDto(0, 600, 200);
        var command = CreateValidCommand() with
        {
            UpperBallJoint = samePoint,
            LowerBallJoint = samePoint,
        };
        var result = _validator.TestValidate(command);

        result.ShouldHaveAnyValidationError()
            .WithErrorMessage("Upper and lower ball joints must not be at the same position");
    }

    [Fact]
    public void NearlyCoincidentBallJoints_WithinTolerance_ShouldFail()
    {
        // Points separated by less than sqrt(1e-6) ~ 0.001mm
        var command = CreateValidCommand() with
        {
            UpperBallJoint = new Point3DDto(0, 600, 200),
            LowerBallJoint = new Point3DDto(0, 600, 200.0005),
        };
        var result = _validator.TestValidate(command);
    ...
    [Fact]
    public void DistinctBallJoints_ShouldPass()
    {
        var command = CreateValidCommand() with
        {
            UpperBallJoint = new Point3DDto(0, 600, 280),
